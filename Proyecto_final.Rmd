...
En base a lo que veniamos viendo, ahora visualizaremos lo mismo del grafico anterior pero desde un grafico de cajas:
...
```{r, echo = FALSE, message = FALSE, fig.width=7, fig.height=3.5, fig.cap = "En esta figura podemos observar un grafico de densidad para la variable afecto_positivo, para los países que corresponden a Oceanía. Observamos que Nueva Zelanda tiene una aproximación a una distribución simétrica y con una media en el entorno de 0.85, en elS caso de Australia podemos ver que su media se encuentra en el entorno de 0.80, ambas estando muy cerca de 1"}
base_felicidad %>% 
           filter(continente == "Oceania") %>% 
           group_by(pais) %>% 
           summarise(afecto_positivo) %>% 
      ggplot(aes(x = afecto_positivo)) + geom_density(aes(fill = pais)) + 
      labs(x = "Afecto Positivo", y = "Densidad", fill = "Pais")
```
...
Por otro lado, nos interesa saber cual es la relacion que existe entre las variables Felicidad y PBI para los paises de Sudamerica seleccionados:
...
```{r, echo = FALSE, fig.cap = "Relacion entre PIB y Felicidad, observamos que hay una relación positiva pero débil, entre el PIB y el puntaje de felicidad; a mayor PIB, mayor puntaje de felicidad." }
